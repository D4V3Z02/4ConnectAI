...
class Lobby:
    ...
    def on_server_click(self, widget):
        ...
        networking.engine.Engine(settings.NetworkEngineMode.JOIN, widget.data['ip'])
    ...
    def update(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
                ...
                elif self.lobby_type == settings.LobbyStates.HOST_LAN_GAME and self.lan_announcer:
                    self.lan_announcer.stop()
                    del self.lan_announcer
                elif self.lobby_type == settings.LobbyStates.JOIN_LAN_GAME and self.lan_discoverer:
                    self.lan_discoverer.stop()
                    del self.lan_discoverer
