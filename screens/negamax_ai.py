...
class GameNegamaxAI(ai.AIGame):
    ...
    def evaluate_next_turn(self, board: dict, depth: int, ai_player: Player) -> int:
        ...
        for column, value in board.items():
            ...
            if chip_row_stop >= 0:
                ...
                possible_moves[column] = self.recursive_search(depth - 1, child_board,
                                                               self.get_other_player(ai_player))
    ...
    def recursive_search(self, depth: int, board: dict, player: Player):
        ...
        for child in possible_moves:
            move_score = max(move_score, -self.recursive_search(depth-1, child,
                                                                self.get_other_player(player)))
        return move_score
